...
Procedure west_lake_actions;

begin

	if (current_verb=VERB_GET) or
		(current_verb=VERB_TAKE) or
			(current_verb=VERB_STEAL) then begin

		if (current_noun=NOUN_BERRIES) then
			print_offset:=
				lake_west_dialog(lake_west_get_berries_message);

		if (current_noun=NOUN_ROCKS) or
			(current_noun=NOUN_STONES) or
			(current_noun=NOUN_PEBBLES) then begin
...
(*
	...
	VERB_LOOK:	begin
		...
		if (current_noun=NOUN_BOAT) or (current_noun=NOUN_DINGHY) then begin
			if game_state.FISH_FED then
				print_offset:=lake_west_dialog(
					lake_west_look_at_boat_gone_message)
			else
				print_offset:=lake_west_dialog(
					lake_west_look_at_boat_message);
		end;
		...
		if (current_noun=NOUN_DUDE) or (current_noun=NOUN_MAN) or
			...
			if game_state.FISH_FED=false then
				print_offset:=lake_west_dialog(
					lake_west_look_at_man_message);

		if (current_noun=NOUN_LAKE) or (current_noun=NOUN_NONE) then begin

			if game_state.FISH_FED then
				print_offset:=lake_west_dialog(
					lake_west_look_at_lake_message_man_gone)
			else
				print_offset:=lake_west_dialog(
		...
		if (current_noun=NOUN_SAND) then begin
			print_offset:=lake_west_dialog(lake_west_look_at_sand_message);
			partial_message_step;
			print_offset:=lake_west_dialog(lake_west_look_at_sand_message2);
			end;
	...
	VERB_TALK:
		if (current_noun=NOUN_DUDE) or (current_noun=NOUN_MAN) or
			...
			if game_state.FISH_FED then
				print_offset:=lake_west_dialog(
					lake_west_talk_man_after_message)
			else
				print_offset:=lake_west_dialog(
					lake_west_talk_man_message);
	...
	VERB_THROW: begin
		...
		if (current_noun=NOUN_FEED) then begin
			...
			if (inventory[ord(CHICKEN_FEED)]) then begin
				...
				if (peasant_y<52) or (peasant_y>120) then begin
					...
					if (game_state.FISH_FED) then begin
						print_offset:=
						lake_west_dialog(
						lake_west_throw_feed_already_message);
					end
					else begin
						{ he's there still }
						{TODO}
				...
				print_offset:=
				lake_west_dialog(lake_west_throw_feed_none_message);
			...
			print_offset:=
				lake_west_dialog(lake_west_skip_stones_message);
		end {NOUN_STONES}
		else
			{ throw anything else }
			if (game_state.FISH_FED) then
				print_offset:=
				lake_west_dialog(lake_west_throw_default_gone_message)
			else
				print_offset:=
				lake_west_dialog(lake_west_throw_default_message);
...
Procedure do_west_lake;

label done_west_lake;
...
begin
 ...
 {	screen_copy(screen_ptr(framebuffer),screen_ptr(background));
	...
	while(level_over=LEVEL_NOT_OVER) do begin
		...
		if level_over <> LEVEL_NOT_OVER then goto done_west_lake;
